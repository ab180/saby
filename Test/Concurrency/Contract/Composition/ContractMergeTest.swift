...
extension Contract.MergedValue2: Equatable where Value0: Equatable, Value1: Equatable {
    public static func == (lhs: Contract.MergedValue2<Value0, Value1>, rhs: Contract.MergedValue2<Value0, Value1>) -> Bool {
        if case .value0(let leftValue) = lhs,
           case .value0(let rightValue) = rhs
        {
            return leftValue == rightValue
        }
        
        if case .value1(let leftValue) = lhs,
